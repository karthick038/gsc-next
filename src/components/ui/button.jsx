...
const Button = React.forwardRef(({ className, variant, size, asChild = false, ...props }, ref) => {
  const Comp = asChild ? "span" : "button" // Fallback to span if asChild to prevent double button, OR we clone.
  // Real Radix Slot merges props. Here we'll just handle valid children manually where possible, 
  // or simply ignore asChild if it's too complex and just render the button.
  // Given user requirement "Remove Radix", we will just render a button. 
  // If asChild is passed (like in AlertDialogTrigger), we should try to render the child.
